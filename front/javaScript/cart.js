  ...
  for (camera of products) {
    ...
    for (let j = 0; j < 10; j++) {
      if (j + 1 == camera.quantity) {
        camera["select"] += `<option selected value="${j + 1}">${
          j + 1
        }</option>`;
      } else {
    ...
    bigTotal.innerHTML = ` <div id="total-price">${resultTotal.toFixed(
      2
    )} €</div>`;
  }
}
...
function deleteItem(element) {
  ...
  if (productList2.length > 1) {
    ...
    resultTotal = (
      Number(resultTotal).toFixed(2) - Number(newSum2[0]).toFixed(2)
    ).toFixed(2);
...
function sendToApi() {
  ...
  let regexAdress = /[0-9 ]{1,3}[a-zA-Z-' ]+[a-zA-Z-' ]+[a-zA-Z'-]+$/.test(
    adress
  );
